...
public:
	...
	SDL_Texture *bmp;
	SDL_Rect rect;
	void video_play(int x, int y, int width, int height, int window_width, int window_height, MediaState *media);
	double synchronize(AVFrame *srcFrame, double pts);
	
	VideoState();
	VideoState(const char* name, SDL_Window *window, SDL_Renderer *renderer);
